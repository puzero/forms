...
namespace dataBase1
{
    public partial class FormAdmin : Form
    {
        ...
        private SqlDataAdapter sqlDataAdapterUsers = null;
        ...
        private void ReloadUsersData()
        {
            try
            {
                dataSetUsers.Tables["Users"].Clear();
                sqlDataAdapterUsers.Fill(dataSetUsers, "Users");

                dataGridViewUsers.DataSource = dataSetUsers.Tables["Users"];
                ...
                for (int i = 0; i < dataGridViewUsers.Rows.Count; i++)
                {
                    ...
                    dataGridViewUsers[3, i] = linkCell;
        ...
        private void LoadUsersData()
        {
            try
            {
                sqlDataAdapterUsers = new SqlDataAdapter("select  id, role, password, 'Delete' as [Command] from Users", sqlConnection);
                sqlCommandBuilder = new SqlCommandBuilder(sqlDataAdapterUsers);
        ...
        private void dataGridViewUsers_UserAddedRow(object sender, DataGridViewRowEventArgs e)
        {
            try
            {
                if (newRowAdding == false)
                {
                    ...
                    int lastRow = dataGridViewUsers.RowCount - 2;

                    DataGridViewRow row = dataGridViewUsers.Rows[lastRow];

                    DataGridViewLinkCell linkCell = new DataGridViewLinkCell();

                    dataGridViewUsers[3, lastRow] = linkCell;
        ...
        private void dataGridViewUsers_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.ColumnIndex == 3)
            {
                ...
                string task = dataGridViewUsers.Rows[e.RowIndex].Cells[3].Value.ToString();
                ...
                if (task == "Delete")
                {
                        ...
                        == DialogResult.Yes)
                    {
                        ...
                        dataSetUsers.Tables["Users"].Rows[rowIndex].Delete();
                        sqlDataAdapterUsers.Update(dataSetUsers, "Users");
                    }
                ...
                else if (task == "Insert")
                {
                    ...
                    int rowIndex = dataGridViewUsers.Rows.Count - 2;

                    DataRow row = dataSetUsers.Tables["Users"].NewRow();



                    row["id"] = dataGridViewUsers.Rows[rowIndex].Cells["id"].Value;
                    row["role"] = dataGridViewUsers.Rows[rowIndex].Cells["role"].Value;
                    row["password"] = dataGridViewUsers.Rows[rowIndex].Cells["password"].Value;


                    dataSetUsers.Tables["Users"].Rows.Add(row);
                    dataSetUsers.Tables["Users"].Rows.RemoveAt(dataSetUsers.Tables["Users"].Rows.Count - 1);
                    dataGridViewUsers.Rows.RemoveAt(dataGridViewUsers.Rows.Count - 2);
                    dataGridViewUsers.Rows[e.RowIndex].Cells[3].Value = "Delete";
                ...
                else if (task == "Update")
                {
                    ...
                    dataSetUsers.Tables["Users"].Rows[r]["id"] = dataGridViewUsers.Rows[r].Cells["id"].Value;
                    dataSetUsers.Tables["Users"].Rows[r]["role"] = dataGridViewUsers.Rows[r].Cells["role"].Value;
                    dataSetUsers.Tables["Users"].Rows[r]["password"] = dataGridViewUsers.Rows[r].Cells["password"].Value;
                    sqlDataAdapterUsers.Update(dataSetUsers, "Users");
                    dataGridViewUsers.Rows[e.RowIndex].Cells[3].Value = "Delete";
                }
        ...
        private void dataGridVieUsers_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                if (newRowAdding == false)
                {
                    ...
                    DataGridViewRow editingRow = dataGridViewUsers.Rows[rowIndex];

                    DataGridViewLinkCell linkCell = new DataGridViewLinkCell();

                    dataGridViewUsers[3, rowIndex] = linkCell;
